      ...
      steps {
        container('maven') {
          script {
            ...
            nxMvn.download(artifact: "org.nuxeo.ecm.platform:nuxeo-api-playground-package:${BUILD_VERSION}:zip")
            nxUtils.postForm(credentialsId: 'connect-prod', url: "${CONNECT_PROD_SITE_URL}marketplace/upload?batch=true",
                form: ["package=@target/nuxeo-api-playground-package-${BUILD_VERSION}.zip"])
          }
        }
      }
    }
